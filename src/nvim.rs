...
fn write_init(dir: &Path) -> anyhow::Result<PathBuf> {
    let path = dir.join("nvim-init.lua");
    let lua = r##"-- Generated by Blackhole on every editor start; edit ~/.config/nvim instead.
vim.o.number = true
...
impl Host {
    ...
    pub fn create(parent: HWND, font_px: i32, unit: i32, accent: COLORREF) -> Option<Host> {
        let exe = find_nvim()?;
        unsafe {
            ...
            let init = write_init(&crate::config::data_dir()).ok()?;
            ...
            Some(Host { hwnd, rpc: h.rpc.clone(), cols: 0, rows: 0, grid: Vec::new(), attrs: HashMap::new(), default_fg: 0, default_bg: 0, cursor: (0, 0), mode: String::new(), viewport: (0, 0, 1), font: HFONT::default(), font_bold: HFONT::default(), font_italic: HFONT::default(), cell_w: h.cell_w, cell_h: h.cell_h, own_tick: 0, mouse_down: false, unit, accent })
        }
    }
    ...
    match msg {
        ...
        WM_CHAR => {
            if let Some(h) = state(hwnd) {
                ...
                if c >= 0x20 && c != 0x7F {
                    let s = char::from_u32(c).map(|ch| ch.to_string()).unwrap_or_default();
                    h.input(&s.replace('<', "<lt>"));
                }
            }
            LRESULT(0)
        }
