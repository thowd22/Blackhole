...
Still open from that list: a third blind question set (~30) and an end-to-end answer eval with the 1.5B; and new from the scale run, chunk-level context can pick a sibling document when two differ only by a rare token ("Corvane-375" for a question about "Corvane-225") — the candidate fix is to treat a rare alphanumeric token as a hard filter rather than a boost.
